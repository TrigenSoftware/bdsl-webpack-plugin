# `transpile-dependencies` example

Usage example with configuration for dependencies transpilation.
...
3) ♻️ Add some tweaks to `webpack.config.js` for dependencies transpilation:
...
function createWebpackConfig(env) {
    return {
        ...
        module:  {
            rules: [{
                test:    /\.js$/,
                exclude: _ => /node_modules/.test(_) && !/\.babel\.js$/.test(_),
                loader:  'babel-loader',
                options: {
                    cacheDirectory: true,
                    presets:        [
...
5) 🎉 Done! Now `index.html` will contain differential script loading:
